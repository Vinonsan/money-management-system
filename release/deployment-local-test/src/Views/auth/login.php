...
<?php if ($step === 'otp'): ?>
    <form method="post" action="<?= BASE_URL ?>/admin/login/verify-otp" class="space-y-5"
          x-data
        ...
        <a href="<?= BASE_URL ?>/admin/login?reset=1" class="block text-center text-sm text-gray-500 hover:text-primary-700">
            Change phone number
        </a>
    </form>
<?php else: ?>
    <form method="post" action="<?= BASE_URL ?>/admin/login/request-otp" class="space-y-4">
        <input type="hidden" name="csrf_token" value="<?= e($_SESSION['csrf_token'] ?? '') ?>">
