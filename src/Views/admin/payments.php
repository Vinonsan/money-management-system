...
<div class="mx-auto max-w-7xl space-y-6 py-2"
    ...
    <div class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <h2 class="text-lg font-bold text-slate-900 mb-1">Step 1: Find User</h2>
        <p class="text-sm text-slate-500 mb-4">Search by name or card number.</p>
        ...
        <div class="flex items-center gap-3">
            ...
            <button type="button" @click="searchUsers()"
                class="rounded-lg bg-emerald-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-emerald-700 transition">
                Search
            </button>
        </div>
        ...
        <div x-show="searchResults.length > 0" class="mt-4">
            <p class="text-xs font-semibold text-slate-500 mb-2" x-text="searchResults.length + ' user(s) found'"></p>
        ...
        <div x-show="searchResults.length === 0 && searched && !selectedUser" class="mt-4 text-center py-6 text-sm text-slate-400">
    ...
    <div x-show="selectedUser" x-cloak class="rounded-2xl border border-slate-200/80 bg-white p-6 shadow-sm">
        <h2 class="text-lg font-bold text-slate-900 mb-1">Step 2: Record Payment</h2>
        <p class="text-sm text-slate-500 mb-4" x-text="'Selected: ' + selectedUser?.name"></p>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
            ...
            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
                ...
                <p class="text-lg font-bold text-amber-600" x-text="paymentInfo?.next_due_month ? formatDate(paymentInfo.next_due_month) : '-'"></p>
            </div>
            <div class="rounded-xl bg-slate-50 p-3 border border-slate-100">
        ...
        <div class="max-w-lg space-y-4">
            ...
            <div>
                ...
                <input type="text" x-model="paymentNotes" placeholder="Any notes..."
                    class="w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/10">
            </div>
        ...
        submitPayment() {
            ...
            .then(r => {
                if (r.success) {
                    ...
                    alert(r.error || 'Failed to record payment.');
                }
            })
