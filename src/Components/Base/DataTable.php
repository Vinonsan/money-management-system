...
final class DataTable
{
    public static function render(array $config): string
    {
        ...
        // SVG Icons
        $sortIconAsc = '<svg class="inline h-3.5 w-3.5 text-emerald-600 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 15l7-7 7 7"/></svg>';
        $sortIconDesc = '<svg class="inline h-3.5 w-3.5 text-emerald-600 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M19 9l-7 7-7-7"/></svg>';
                ...
                <form method="get" class="flex items-center gap-3 w-full sm:w-auto">
                    <div class="relative flex-1 sm:w-80">
                        ...
                        <input type="text" name="search" value="{$search}"
                            ...
                            class="w-full rounded-xl border border-slate-200 bg-slate-50/50 py-2 pl-10 pr-4 text-sm text-slate-700 placeholder-slate-400 transition-all focus:border-emerald-500 focus:bg-white focus:outline-none focus:ring-4 focus:ring-emerald-500/10">
                    </div>
                    <button type="submit"
                        class="rounded-xl bg-emerald-600 px-4 py-2 text-sm font-medium text-white shadow-sm shadow-emerald-600/20 hover:bg-emerald-700 active:scale-[0.98] transition-all">
                        Search
                    </button>
                    {$clearLink}
                </form>
            ...
            if ($sortable && $field) {
                ...
                <th class="px-6 py-4 {$textAlign} text-xs font-bold uppercase tracking-wider text-slate-500 {$width}">
                    <a href="{$baseUrl}?sort={$field}&dir={$nextDir}{$searchParam}"
                       class="group inline-flex items-center hover:text-emerald-600 transition-colors">
                        {$label} {$icon}
                    </a>
                </th>
                HTML;
        ...
        <div class="flex flex-wrap items-center justify-between gap-4 px-6 py-4 border-t border-slate-100 bg-slate-50/30">
            <div class="flex items-center gap-3 text-xs text-slate-500 font-medium">
                ...
                <select onchange="window.location.href='{$baseUrl}?per_page='+this.value+'{$searchQuery}{$sortQuery}'"
                    class="rounded-lg border border-slate-200 bg-white px-2.5 py-1 text-xs text-slate-700 font-medium shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/10">
                    {$perPageOptionsHtml}
                </select>
